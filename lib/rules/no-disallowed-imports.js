...
module.exports = {
  meta: {
    ...
    messages: {
      "no-disallowed-imports": "{{specifier}} is should NOT be imported",
    },
  },
  create(context) {
    /**
      ...
      ImportDeclaration(node) {
        ...
        const searchTheImportPath = (
          ...
          const specifier = node.specifiers.find(
            ({ imported }) => specifiers.includes(imported.name) // FIXME
          );
          if ((theSpecifier, minimatch(modulePath, path))) {
        ...
        const isAllowed =
          ...
          Boolean(disallow.find((pattern) => minimatch(fileSource, pattern)));
        if (isAllowed) return;
