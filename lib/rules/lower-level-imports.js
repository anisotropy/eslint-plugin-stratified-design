...
module.exports = {
  ...
  create(context) {
    ...
    const checkHasProperLevelFromName = (modulePath) => {
      ...
      if (!options.useLevelNumber) return "not-lower-level";

      const fileSegments = toSegments(filePath);
      const moduleSegments = toSegments(modulePath);

      if (findParent(fileSegments) !== findParent(moduleSegments)) {
        return "not-lower-level";
      }

      if (isNodeModule(modulePath)) {
        return "not-registered:module";
      }

      const fileLevel = findLevel(fileSegments);
      if (fileLevel < 0) {
        return "not-registered:file";
      }
    ...
    return {
      ImportDeclaration(node) {
        ...
        const messageId = (() => {
          ...
          return messageId && checkHasProperLevelFromName(modulePath)
            ? messageId
            : null;
        })();
