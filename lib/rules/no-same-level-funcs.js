...
const hasArrFunction = (elements, levels) => {
  return elements.reduce((hasFunc, element) => {
    ...
    if (type === "FunctionExpression" || type === "ArrowFunctionExpression")
      return true;
    if (type === "ObjectExpression") return hasObjFunction(element.properties, levels);
    if (type === "ArrayExpression") return hasArrFunction(element.elements, levels);
...
const isCalleeLowerLevel = (sourceCode, levels) => {
  ...
  return (node, calleeName) => {
    ...
    console.log('--> calleeName', calleeName, 'calleeLevel', calleeLevel)
